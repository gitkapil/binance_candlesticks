                ...
                if kline.get('x'):  # kline closed
                    ...
                    class BinanceWebSocketClient:
                        ...
                        def collect_trades_for_interval(self, interval):
                            asyncio.run(self._collect_trades_for_interval(interval))

                        async def _collect_trades_for_interval(self, interval):
                            self.trades = []
                            uri = f"{self.ws_url}?streams={self.trade_stream}"
                            async with websockets.connect(uri, ssl=self.ssl_context) as ws:
                                start_time = None
                        ...
                        def collect_kline_for_interval(self):
                            asyncio.run(self._collect_kline_for_interval())

                        async def _collect_kline_for_interval(self):
                            self.klines = []
                            uri = f"{self.ws_url}?streams={self.kline_stream}"
                            async with websockets.connect(uri, ssl=self.ssl_context) as ws:
                                while True:
                        ...
                        def collect_multiple_intervals(self, count, interval):
                            asyncio.run(self._collect_multiple_intervals(count, interval))
